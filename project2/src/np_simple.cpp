...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        cout<<"A client "<<inet_ntoa(client_addr.sin_addr)<<" has connected via port num "<<client_addr.sin_port<<" with socket_fd: "<<cli_sock<<endl;
        ...
        else // parent process will wait it child
        {
            wait(nullptr);
            close(cli_sock);
            cout<<"client: "<<cli_sock<<" exit!"<<endl;
        }
    }
